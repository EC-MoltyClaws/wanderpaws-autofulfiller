...
def fetch_reships_from_file(filepath: str = "reships.json") -> list[list]:
    """
    Reads the reships.json file generated by the GitHub CLI in the workflow.
    Expects the issue body to contain a strict JSON object mapping to the 13 Excel columns.
    Returns a list of rows (lists) ready to be appended to the Excel data.
    """
    ...
    for issue in issues:
        ...
        try:
            ...
            # The JSON keys must match the expected Excel headers exactly, or provide defaults
            row = [
                order_data.get("Name", f"#{issue.get('title')}"),
                order_data.get("Email", ""),
                order_data.get("Lineitem name", ""),
                order_data.get("Shipping Name", ""),
                order_data.get("Shipping Street", ""),
                order_data.get("Shipping Address1", ""),
                order_data.get("Shipping Address2", ""),
                order_data.get("Shipping Company", ""),
                order_data.get("Shipping City", ""),
                order_data.get("Shipping Zip", ""),
                order_data.get("Shipping Province", ""),
                order_data.get("Shipping Country", ""),
                order_data.get("Shipping Phone", ""),
            ]
            reship_rows.append(row)
            print(f"Parsed reship payload from Issue #{issue.get('number')}")
        except Exception as e:
            print(f"Error parsing JSON from issue #{issue.get('number')}: {e}")
...
def main() -> None:
    ...
    try:
        ...
        reship_rows = fetch_reships_from_file()
        
        if not orders and not reship_rows:
        ...
        # Inject Reships from GitHub Issues (pre-flattened format)
        if reship_rows:
            rows.extend(reship_rows)
            label += f" (+{len(reship_rows)} reships)"
        
        buf = generate_excel(rows)

        send_telegram_document(buf, filename, f"*WanderPaws {label}*\n`{len(orders) + len(reship_rows)}` total order(s)")
